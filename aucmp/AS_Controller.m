...
-(instancetype)initWithFiles:(NSArray<NSString*>*)files params:(NSDictionary*)params
{
    if (self = [super init])
    {
        ...
        if (params[VERBOSE_KEY])
            self.verbose = YES;
        id val;
        if ((val = params[MAX_THRESHOLD_KEY]))
            self.maxThreshold = [val floatValue];
        if ((val = params[AVG_THRESHOLD_KEY]))
            self.avgThreshold = [val floatValue];
    }
    return self;
}
...
-(BOOL)compareFiles
{
    ...
    for (int i = 0;i < afile1.noChannels;i++)
    {
        ...
        if (thismax > self.maxDiff)
            self.maxDiff = thismax;
        
        Float32 sum;
        vDSP_sve(buf2, 1, &sum, afile1.noFrames);
        totDiff += sum;
    }
    self.avg = totDiff / (afile1.noFrames * afile1.noChannels);
    if (self.maxDiff == 0 && self.avg == 0)
        printf("Exact match");
    else
        printf("Partial match - Max difference: %g Avg difference: %g\n",self.maxDiff,self.avg);
    return YES;
}
